...
class Socks_IMAP4(IMAP4):
  ...
  def __init__(self, host, port=IMAP4_PORT, proxy_addr=None, proxy_port=None,
    ...
    self.proxy_type = Socks_IMAP4.PROXY_TYPES[proxy_type.lower()]

    IMAP4.__init__(self, host, port)

  def _create_socket(self):
    if self.proxy_addr is None:
      return IMAP4._create_socket(self)
    else:
...
class Socks_IMAP4_SSL(Socks_IMAP4):
  def __init__(self, host='', port=IMAP4_SSL_PORT, keyfile=None, certfile=None,
    ...
    Socks_IMAP4.__init__(self, host, port, proxy_addr=proxy_addr, proxy_port=proxy_port,
		        rdns=rdns, username=username, password=password, proxy_type=proxy_type)

  def _create_socket(self):
    sock = Socks_IMAP4._create_socket(self)
